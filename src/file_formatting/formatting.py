...
def add_table_of_contents(doc, heading_paragraph, structure=None):
    """
    Builds a purely static Table of Contents with dot leaders and right-aligned page numbers.
    Entries are generated by pre-scanning the document structure using the optimized page estimator.
    This does NOT use Word field codes, preventing Word from corrupting the layout upon open.
    """
    if not structure:
        return
...
def add_list_of_figures(doc, heading_paragraph, structure=None):
    """
    Builds a purely static List of Figures with dot leaders and right-aligned page numbers.
    Prevents Word from erroneously reporting "No table of figures entries found" due to missing SEQ fields.
    """
    if not structure:
        return
